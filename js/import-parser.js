...
function runImport(text) {
  ...
  function parseDate(str){
    ...
    const mY=str.match(/^(january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|jun|jul|aug|sep|sept|oct|nov|dec)\.?\s+(\d{4})$/i);
    if(mY){const mon=mY[1].toLowerCase().replace(/\.$/,'');const mi=monthNames.findIndex(m=>mon===m||mon.startsWith(m));if(mi>=0)return `${mY[2]}-${String(mi+1).padStart(2,'0')}`;}
    if(/^\d{4}$/.test(str))return str;
    const mmYY=str.match(/^(\d{1,2})[\/\-](\d{4})$/);if(mmYY)return `${mmYY[2]}-${mmYY[1].padStart(2,'0')}`;
    return str;
  }
  function extractDateRange(line){
    const mYmY=line.match(/([A-Za-z]+\.?\s+\d{4})\s*[-–—]\s*([A-Za-z]+\.?\s+\d{4}|present)/i);if(mYmY)return{start:parseDate(mYmY[1]),end:parseDate(mYmY[2])};
    const yY=line.match(/(\d{4})\s*[-–—]\s*(\d{4}|present)/i);if(yY)return{start:parseDate(yY[1]),end:parseDate(yY[2])};
    const sy=line.match(/\b(\d{4})\b/);if(sy)return{start:'',end:parseDate(sy[1])};
    return null;
  }
  ...
  if(eduIdx>=0){
    ...
    getBlockBetween(eduIdx).forEach(line=>{
      ...
      if(dr&&(line.includes('–')||line.includes('-')||line.includes('—')||/\d{4}/.test(line))&&cur){
        ...
        if(dr.end){if(/present/i.test(String(dr.end))){cur.endDate='Present';cur.expected=true;}else cur.endDate=dr.end;if(/expected/i.test(line))cur.expected=true;}
        if(dr.start)cur.dateMode='range';
      }else if(isGPA&&cur){const g=line.match(/[\d.]+/);if(g)cur.gpa=g[0];}
      else if(isBullet&&cur){}
      else if(isCity&&cur){cur.location=line;}
      else if(isDeg&&cur){const dm=line.match(/(B\.?A\.?|B\.?S\.?|B\.?F\.?A\.?|M\.?A\.?|M\.?S\.?|M\.?F\.?A\.?|Ph\.?D\.?|Bachelor of [A-Za-z\s]+|Master of [A-Za-z\s]+|Associate[A-Za-z\s]*|Diploma[A-Za-z\s]*|Certificate[A-Za-z\s]*)/i);if(dm){cur.degree=dm[0].trim();const rest=line.slice(line.indexOf(dm[0])+dm[0].length).replace(/^[\s,in]+/i,'').trim();if(rest)cur.field=rest;}else cur.degree=line;}
      else if(line.length>3){if(cur&&cur.institution)eduSec.items.push(cur);cur={institution:line,degree:'',field:'',location:'',startDate:'',endDate:'',gpa:'',dateMode:'expected',expected:false};}
    });
  ...
  if(expIdx>=0){
    ...
    let cur=null;let dateAssigned=false;
    getBlockBetween(expIdx).forEach(line=>{
      const isBullet=/^[•\-–*▪]\s/.test(line);
      const isCity=/^[A-Z][a-zA-Z\s]+,\s*(?:[A-Z]{2}|[A-Z][a-z]{2,})$/.test(line);
      const dr=extractDateRange(line);
      const looksDate=dr&&(/[-–—]/.test(line)||/\d{4}/.test(line))&&line.length<40;
      if(isBullet&&cur){cur.description=(cur.description?cur.description+'\n':'')+line.replace(/^[•\-–*▪]\s*/,'').trim();}
      else if(looksDate&&cur){if(dr.start)cur.startDate=dr.start;if(dr.end){if(/present/i.test(String(dr.end))){cur.endDate='';cur.current=true;}else cur.endDate=dr.end;}dateAssigned=true;}
      else if(isCity&&cur&&!cur.location){cur.location=line;}
      else{
        const pipe=line.match(/^(.+?)\s*[|\/]\s*(.+)$/);
        const at=line.match(/^(.+?)\s+(?:at|@)\s+(.+)$/i);
        if(pipe){if(cur&&cur.company)expSec.items.push(cur);cur={company:pipe[1].trim(),position:pipe[2].trim(),location:'',startDate:'',endDate:'',description:'',current:false};dateAssigned=false;}
        else if(at){if(cur&&cur.company)expSec.items.push(cur);cur={company:at[2].trim(),position:at[1].trim(),location:'',startDate:'',endDate:'',description:'',current:false};dateAssigned=false;}
        else if(!cur||(cur.company&&cur.position&&dateAssigned)){if(cur&&cur.company)expSec.items.push(cur);cur={company:line,position:'',location:'',startDate:'',endDate:'',description:'',current:false};dateAssigned=false;}
        else if(cur.company&&!cur.position)cur.position=line;
      }
    });
  ...
  if(exhibIdx>=0){
    ...
    getBlockBetween(exhibIdx).forEach(line=>{
      ...
      if(isWorks&&cur)cur.workTitles=line.replace(/^works?:\s*/i,'');
      else if(looksDate&&cur){if(dr.start)cur.startDate=dr.start;if(dr.end)cur.endDate=dr.end;}
      else if(isCity&&cur&&!cur.location)cur.location=line;
      else if(line.length>2){if(!cur)cur={exhibitionTitle:line,venue:'',location:'',startDate:'',endDate:'',workTitles:''};else if(!cur.venue&&cur.exhibitionTitle)cur.venue=line;else{if(cur.exhibitionTitle)exhibSec.items.push(cur);cur={exhibitionTitle:line,venue:'',location:'',startDate:'',endDate:'',workTitles:''};}}
    });
  ...
  if(skillIdx>=0){
    ...
    getBlockBetween(skillIdx).forEach(line=>{
      ...
      if(ci>0&&ci<35)skillSec.categories.push({category:line.slice(0,ci).trim(),skillsText:line.slice(ci+1).trim()});
      else if(/^[•\-–*▪]\s/.test(line)){const skill=line.replace(/^[•\-–*▪]\s*/,'').trim();if(skillSec.categories.length&&!skillSec.categories[skillSec.categories.length-1].category)skillSec.categories[skillSec.categories.length-1].skillsText+=', '+skill;else skillSec.categories.push({category:'',skillsText:skill});}
      else if(line.includes(','))skillSec.categories.push({category:'',skillsText:line});
      else if(line.length>1){if(skillSec.categories.length&&!skillSec.categories[skillSec.categories.length-1].category)skillSec.categories[skillSec.categories.length-1].skillsText+=', '+line;else skillSec.categories.push({category:'',skillsText:line});}
    });
  }
