...
function load(){
  try{
    ...
    if(ls) {
      lineSpacing = parseFloat(ls);
      window.lineSpacing = lineSpacing;
    }
  }catch(_){}
}
...
// expose inline-handler functions to global scope for published builds
window.setNameFont = setNameFont;
window.setBodyFont = setBodyFont;
window.setLineSpacing = setLineSpacing;
...
window.deleteSec = deleteSec;
window.addItem = addItem;
window.addCat = addCat;
window.delItem = delItem;
window.delCat = delCat;
window.onInput = onInput;
window.mypChange = mypChange;
window.setEduDateMode = setEduDateMode;
window.setEduExpected = setEduExpected;
window.setExpCurrent = setExpCurrent;
