...
const THEME_TITLES = { light:'Light mode', dark:'Dark mode', dark2:'Dark mode (dark resume)' };
...
load();applySliders();renderSidebar();
const initSec=sections.find(s=>s.type==='personal')?.id||sections[0]?.id||null;
if(initSec)selectSection(initSec);
renderPreview();setTimeout(fitPreview,100);
