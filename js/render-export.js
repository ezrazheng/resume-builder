...
function formatDate(val){
  ...
  if(parts.length===2&&parts[0].length===4&&parseInt(parts[1])>=1&&parseInt(parts[1])<=12)return MONTHS[parseInt(parts[1])]+' '+parts[0];
  return val;
}

function renderPreview(){
  const p=personalInfo;let html='';
  if(p.fullName)html+=`<div class="r-name" style="font-size:${nameFontSize}pt">${esc(p.fullName)}</div>`;
  html+=`<hr class="r-rule">`;
  const contact=[p.location,p.email,p.phone].filter(Boolean);
  const links=[p.linkedin,p.website].filter(Boolean).map(l=>l.replace(/^https?:\/\/(www\.)?/,''));
  if(contact.length)html+=`<div class="r-contact" style="margin-bottom:${links.length?2:14}px">${contact.map(esc).join(' &bull; ')}</div>`;
  if(links.length)html+=`<div class="r-contact" style="margin-bottom:14px">${links.map(esc).join(' &bull; ')}</div>`;
  ...
  sections.forEach(sec=>{
    ...
    if(sec.type==='education'&&sec.items?.length){
      ...
      sec.items.forEach(item=>{
        ...
        let d='';
        if(mode==='range')d=[item.startDate,item.endDate].filter(Boolean).map(formatDate).join(' \u2013 ');
        else{const ds=item.endDate?formatDate(item.endDate):'';d=ds?(item.expected?'expected '+ds:ds):'';}
        const deg=[item.degree,item.field].filter(Boolean).join(', ')+(item.gpa?` (GPA: ${item.gpa})`:'');
        html+=`<div class="r-entry"><div class="r-row"><span class="r-left">${esc(item.institution||'')}</span><span class="r-right">${esc(item.location||'')}</span></div><div class="r-row"><span class="r-sub-left">${esc(deg)}</span><span class="r-sub-right">${esc(d)}</span></div></div>`;
      });
      html+=`</div>`;
    }
    if(sec.type==='experience'&&sec.items?.length){
      ...
      sec.items.forEach(item=>{
        ...
        html+=`<div class="r-entry"><div class="r-row"><span class="r-left">${esc(item.company||'')}</span><span class="r-right">${esc(item.location||'')}</span></div><div class="r-row"><span class="r-sub-left">${esc(item.position||'')}</span><span class="r-sub-right">${esc(d)}</span></div>`;
        if(item.description){const ls=item.description.split('\n').filter(l=>l.trim());if(ls.length)html+=`<div class="r-bullets">${ls.map(l=>`<div class="r-bullet"><span style="flex-shrink:0;margin-top:.15em">&bull;</span><span>${esc(l.trim())}</span></div>`).join('')}</div>`;}
        html+=`</div>`;
      });
      html+=`</div>`;
    }
    if(sec.type==='exhibitions'&&sec.items?.length){
      ...
      sec.items.forEach(item=>{
        ...
        html+=`<div class="r-entry"><div class="r-row"><span class="r-left">${esc(item.exhibitionTitle||'')}</span><span class="r-right">${esc(item.location||'')}</span></div><div class="r-row"><span class="r-sub-left">${esc(item.venue||'')}</span><span class="r-sub-right">${esc(d)}</span></div>${item.workTitles?`<div class="r-plain">Works: ${esc(item.workTitles)}</div>`:''}</div>`;
      });
      html+=`</div>`;
    }
    if(sec.type==='skills'&&sec.categories?.length){
      ...
      sec.categories.forEach(cat=>{
        const skills=(cat.skillsText||'').split(',').map(s=>s.trim()).filter(Boolean);
        if(!skills.length)return;
        html+=`<div class="r-skill-line">${cat.category?`<span class="r-skill-cat">${esc(cat.category)}: </span>`:''}${esc(skills.join(', '))}</div>`;
      });
      html+=`</div>`;
    }
    if(sec.type==='custom'&&sec.items?.length){
      ...
      sec.items.forEach(item=>{
        ...
        if(item.subtitle&&item.title)html+=`<div class="r-row"><span class="r-sub-left">${esc(item.title)}</span><span class="r-sub-right">${esc(d)}</span></div>`;
        if(item.description){const ls=item.description.split('\n').filter(l=>l.trim());if(ls.length)html+=`<div class="r-bullets">${ls.map(l=>`<div class="r-bullet"><span style="flex-shrink:0;margin-top:.15em">&bull;</span><span>${esc(l.trim())}</span></div>`).join('')}</div>`;}
        html+=`</div>`;
      });
      html+=`</div>`;
  ...
  const sheetW=paperSize==='a4'?794:816,sheetH=paperSize==='a4'?1123:1056;
  const padX=72,padTop=54,padBot=54,innerH=sheetH-padTop-padBot;
  const sheet=document.getElementById('resumeSheet');
  const msr=document.createElement('div');
  msr.style.cssText=`position:fixed;left:-9999px;top:0;width:${sheetW}px;font-family:'Times New Roman',Times,serif;font-size:${bodyFontSize}pt;line-height:1.5;padding:${padTop}px ${padX}px ${padBot}px;box-sizing:border-box;color:#111;visibility:hidden;pointer-events:none`;
  msr.innerHTML=html||'<div class="r-empty">Fill in your information on the left.</div>';
  document.body.appendChild(msr);
  const pages=[[]];let usedH=0;
  Array.from(msr.children).forEach(child=>{
    const h=child.getBoundingClientRect().height||child.offsetHeight;
    if(usedH+h>innerH&&usedH>0){pages.push([]);usedH=0;}
    pages[pages.length-1].push(child.outerHTML);usedH+=h;
  });
  document.body.removeChild(msr);
  sheet.innerHTML='';
  pages.forEach(group=>{
    const page=document.createElement('div');page.className=`rpage ${paperSize}`;
    const inner=document.createElement('div');inner.className='rpage-inner';
    inner.style.cssText=`font-size:${bodyFontSize}pt;line-height:1.5;padding:${padTop}px ${padX}px ${padBot}px;box-sizing:border-box`;
...
function fitPreview(){
  ...
  const sheetW=paperSize==='a4'?794:816,sheetH=paperSize==='a4'?1123:1056;
  const areaW=area.clientWidth-40,scale=Math.min(1,areaW/sheetW);
...
function exportPDF(){
  ...
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${personalInfo.fullName||'Resume'}</title><style>
    *{box-sizing:border-box;margin:0;padding:0}body{font-family:'Times New Roman',Times,serif;color:#111;background:#fff}
    ...
    .print-page{width:${sheetW};min-height:${sheetH};padding:54px 72px;font-size:${bodyFontSize}pt;line-height:1.5}.pb{page-break-after:always}
    ...
    .r-entry{margin-bottom:9px}.r-row{display:flex;justify-content:space-between;align-items:baseline}
...
async function exportDOCX(){
  ...
  try{
    ...
    const lineVal=Math.round(bodyFontSize*1.5*20),LS={line:lineVal,lineRule:'exact'};
