...
function renderPreview() {
  ...
  msr.style.cssText = `position:fixed;left:-9999px;top:0;width:${sheetW}px;font-family:'Times New Roman',Times,serif;font-size:${bodyFontSize}pt;line-height:${getLineSpacing()};padding:${padTop}px ${padX}px ${padBot}px;box-sizing:border-box;color:#111;visibility:hidden;pointer-events:none`;
  msr.innerHTML = html || '<div class="r-empty">Fill in your information on the left.</div>';
  const msrStyle = document.createElement('style');
  msrStyle.textContent = `.r-entry{margin-bottom:${Math.round(bodyFontSize * getLineSpacing() * 0.6)}px}.r-section{margin-bottom:${Math.round(bodyFontSize * getLineSpacing() * 0.8)}px}*{line-height:${getLineSpacing()}!important}`;
  msr.insertBefore(msrStyle, msr.firstChild);
  ...
  pages.forEach((group) => {
    ...
    inner.style.cssText = `font-size:${bodyFontSize}pt;line-height:${getLineSpacing()};padding:${padTop}px ${padX}px ${padBot}px;box-sizing:border-box;`;
    inner.innerHTML = group.join('') || '<div class="r-empty">Fill in your information on the left.</div>';
    const styleTag = document.createElement('style');
    styleTag.textContent = `
      .rpage-inner *{line-height:${getLineSpacing()}!important}
      .rpage-inner .r-entry{margin-bottom:${Math.round(bodyFontSize * getLineSpacing() * 0.6)}px}
      .rpage-inner .r-section{margin-bottom:${Math.round(bodyFontSize * getLineSpacing() * 0.8)}px}
    `;
...
function exportPDF(){
  ...
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${personalInfo.fullName||'Resume'}</title><style>
    *{box-sizing:border-box;margin:0;padding:0}body{font-family:'Times New Roman',Times,serif;color:#111;background:#fff}
    ...
    .print-page{width:${sheetW};min-height:${sheetH};padding:54px 72px;font-size:${bodyFontSize}pt;line-height:${getLineSpacing()}}.pb{page-break-after:always}
    ...
    .r-entry{margin-bottom:${Math.round(bodyFontSize * getLineSpacing() * 0.6)}px}.r-row{display:flex;justify-content:space-between;align-items:baseline}
    *{line-height:${getLineSpacing()}!important}
...
async function exportDOCX(){
  ...
  try{
    ...
    const lineVal=Math.round(bodyFontSize*getLineSpacing()*20),LS={line:lineVal,lineRule:'exact'};
